      ...
      <Styled.SuccessOrError color={color}>
        {errorOrSuccess === "success" ? (
          <>
            ...
            <p>
              Placa do veículo registrado: <span>{plate}</span>
            </p>
            <p>
              Código de reserva: <span>{reservation}</span>
            </p>
          </>
        ) : (
          <h4>{errorMSg}</h4>
      ...
      <Styled.Container>
        <form onSubmit={handleSubmit}>
          ...
          <Styled.Button type="submit">
            {loading ? <ClipLoader color="fff" /> : "Confirmar Entrada"}
          </Styled.Button>
        </form>
      </Styled.Container>
      {errorOrSuccess === "success" ? (
        <SuccessOrError color="#17cc92" />
      ) : (
